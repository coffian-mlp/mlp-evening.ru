-- A3 (MLP-228): таблица bot_commands + seed.
-- Раньше DDL и seed гонялись в LLMManager::processTrigger на КАЖДЫЙ вызов бота.
-- Теперь схема живёт здесь (одноразово через migrate.php), владелец данных — BotCommandManager.
-- Идемпотентно: CREATE IF NOT EXISTS + INSERT IGNORE (UNIQUE command_prefix). На проде,
-- где таблица уже создана и засеяна старым кодом, миграция — no-op.
