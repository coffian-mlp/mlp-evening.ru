...
<div class="card">
    ...
    <form method="post" action="/api.php">
        ...
        <div class="form-group">
            <label style="display: flex; align-items: center; cursor: pointer;">
                <input type="hidden" name="stream_command_enabled" value="0">
                <input type="checkbox" name="stream_command_enabled" value="1" <?= $config->getOption('stream_command_enabled', 0) ? 'checked' : '' ?> style="width: auto; margin-right: 10px;">
                <strong>Отвечать на команды стрима</strong>
            </label>
        </div>

        <div class="form-group">
            <label class="form-label">ID владельца трансляции</label>
            <input type="number" name="stream_command_owner_id" value="<?= htmlspecialchars($config->getOption('stream_command_owner_id', 1)) ?>" class="form-input">
            <p style="font-size: 0.85em; color: #666; margin-top: 4px;">Только его команды подтверждаются; ему бот отвечает безусловно.</p>
        </div>

        <div class="form-group">
            <label class="form-label">Кулдаун отказов остальным (сек, 0 = не отвечать чужим)</label>
            <input type="number" name="stream_command_cooldown" value="<?= htmlspecialchars($config->getOption('stream_command_cooldown', 120)) ?>" class="form-input">
