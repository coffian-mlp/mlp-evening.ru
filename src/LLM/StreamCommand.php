...
class StreamCommand {
    ...
    /** Запасные объявления перерыва, если LLM промолчала (MLP-311). */
    private const FALLBACK_BREAK = [
        'Серия закончилась — уходим на перерыв! Возвращайтесь через несколько минут 🎵',
        'Всё, серия доиграла. Небольшой перерыв — не расходитесь далеко!',
        'Перерыв! Разомните копытца, а я пока поставлю музыку 🎶',
    ];
    ...
    private function handleAutoEvent(string $event, LLMManager $llm, ?callable $ask): void {
        ...
        if ($text === '') {
            // Объявление перерыва обязано прозвучать (MLP-311): молчание провайдера
            // или вырезанный санитайзером ответ не должны оставлять зрителей в неведении.
            $text = self::FALLBACK_BREAK[array_rand(self::FALLBACK_BREAK)];
        }
        $llm->botSay($text);
    }
