...
class LyraMemory {
    ...
    public function handleRemember(array $command, array $contextData): bool {
        ...
        if ($kind === 'dossier') {
            ...
            $fallback = sprintf($templates[array_rand($templates)], $targetLabel, $id, $truncNote);
            // MLP-317: живое подтверждение; номер и «ник (логин)» обязательны — адрес и прозрачность.
            $this->llm->botSayLive(
                "Модератор @$username командой /запомни добавил запись №$id в твоё досье о пользователе $targetLabel: «" . mb_substr($payload, 0, 200) . "». "
                . "Запись уже сохранена.$truncNote Подтверди @$username одной-двумя фразами в своём стиле — можно порадоваться новому знанию. "
                . "ОБЯЗАТЕЛЬНО укажи номер записи в формате №$id и имя $targetLabel. Не задавай вопросов.",
                $fallback, [], "№$id"
            );
        } else {
            $templates = [
            ...
            $fallback = sprintf($templates[array_rand($templates)], $id, $truncNote);
            $this->llm->botSayLive(
                "Модератор @$username командой /запомни добавил в твой сундук мемов запись №$id: «" . mb_substr($payload, 0, 200) . "». "
                . "Запись уже сохранена.$truncNote Подтверди одной-двумя фразами в своём стиле — можно оценить мем. "
    ...
    public function handleForget(array $command, array $contextData): bool {
        ...
        // (в живую инструкцию содержимое тоже НЕ передаётся — модели нечего разгласить).
        $templates = [
            "Запись №%d забыта. Как будто и не было!",
            "Готово, №%d вычеркнута из памяти.",
            "Пуф! Записи №%d больше нет.",
        ];
        $fallback = sprintf($templates[array_rand($templates)], $id);
        $this->llm->botSayLive(
            "Модератор @$username командой /забудь стёр запись №$id из твоей памяти (содержимое не разглашается). "
            . "Запись уже удалена. Подтверди одной-двумя фразами в своём стиле. "
            . "ОБЯЗАТЕЛЬНО укажи номер в формате №$id. НЕ пытайся вспомнить или угадать, что было в записи. Не задавай вопросов.",
            $fallback, [], "№$id"
        );
        return true;
    }
