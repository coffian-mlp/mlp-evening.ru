...
class LLMManager {
    ...
    public function processTrigger($triggerType, $contextData = []) {
        ...
        $db->query("CREATE TABLE IF NOT EXISTS `bot_commands` (
        ...
        $res = $db->query("SELECT COUNT(*) as c FROM `bot_commands`");
        if ($res && $res->fetch_assoc()['c'] == 0) {
            $db->query("INSERT IGNORE INTO `bot_commands` (`command_prefix`, `description`, `handler_type`, `system_prompt`, `is_active`) VALUES
