...
class LLMManager {
    ...
    public function processTrigger($triggerType, $contextData = []) {
        ...
        if ($triggerType === 'mention') {
            ...
            $instruction = "Проанализируй последние сообщения. Если нужно что-то сказать (разрядить обстановку, ответить на вопрос, поддержать беседу) - напиши ответ. Если встревать не стоит - ответь ровно одним словом: SILENCE, но не нужно вообще молчать постоянно. Старайся поддерживать беседу в чате, даже если к тебе явно никто не обращается - это нормально. "
                . "Помни: в чате несколько человек — если подхватываешь чью-то реплику, обратись к автору через @Имя и не приписывай его слова другим.";
    ...
    private function askWithFallback($context, $prompt, string $logKind = 'chat', ?int $deadlineSec = null, bool $fast = false) {
        ...
        // MLP-330: Anthropic-модели получают подряд идущие user-сообщения одним ходом и принимают
        // чат за монолог одного человека («твой термос» — Пшенице про термос Коффиана, 19.09).
        // Явное правило про групповой чат — единственное, что не зависит от провайдера.
        $prompt .= "\n[Чат]: это групповой чат, участников несколько. Каждая строка «[ЧЧ:ММ] Имя: …» — реплика ОТДЕЛЬНОГО человека; "
        ...
        foreach (($fast ? $this->fastProviders : $this->providers) as $provider) {
            ...
            try {
                ...
                // MLP-351: реплика с картинкой в тексте — выдумка (26.09: «Нарисовала по памяти» со ссылкой
                // в никуда): настоящие рисунки код приклеивает к подписи после ответа. Отбрасываем целиком —
                // текст вокруг выдуманной картинки тоже неправда. Следующий провайдер, иначе фоллбек вызывающего.
                if ($logKind === 'chat' && $clean !== null && ResponseSanitizer::hasImage($clean)) {
                    error_log("askWithFallback: ответ с выдуманной картинкой отброшен: " . json_encode(mb_substr($clean, 0, 120), JSON_UNESCAPED_UNICODE));
                    continue;
                }
