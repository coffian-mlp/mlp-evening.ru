...
class ReminderCommand {
    ...
    public function handle(array $command, array $contextData): bool {
        ...
        $this->llm->botSayLive(
            ...
            . "Подтверди @$username живой репликой, как в обычной беседе, — НЕ служебным уведомлением. "
            . "Не используй канцелярские конструкции вида «напоминание №N сработает в…» — вместо этого вплети "
            . "номер №$id и время ($when) в текст естественно, например «окей, $when напомню — это у нас №$id». "
            . "Можно обыграть суть просьбы. Не задавай вопросов.",
            $fallback, [], "№$id"
        );
        return true;
    }
