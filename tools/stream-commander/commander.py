...
Команды в чате (только от пользователя OWNER_ID, регистр не важен):
    Клод включи кино      -> сцена кино, играет серия по счётчику (resume после паузы)
    Клод следующая серия  -> счётчик+1, серия с начала
    Клод предыдущая серия -> счётчик-1, серия с начала
    Клод включи перерыв   -> пауза кино + сцена перерыва
    Клод включи начало / конец / ютуб -> соответствующие сцены
"""
...
TRIGGER = re.compile(r"^\s*клод[\s,:!]*", re.IGNORECASE)
