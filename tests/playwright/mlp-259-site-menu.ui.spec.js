...
test('гость: горизонтальное меню в шапке /schedule, активный пункт (MLP-259)', async ({ page, context, browser }) => {
  // v4.10.2: состав меню — контент админа (наборы header/burger переключаемы),
  // поэтому тест сам создаёт себе header-пункт, а не полагается на текущие данные.
  test.skip(!IS_ADMIN, 'нужен MLP_ADMIN=1 (тест создаёт временный пункт меню)');
  await login(page);
  const marker = 'PW259H-' + Date.now();
  ...
  try {
    // Смотрим ГОСТЕМ (чистый контекст без сессии)
    const guest = await browser.newContext({ ignoreHTTPSErrors: true, viewport: { width: 1600, height: 900 } });
    const gpage = await guest.newPage();
    await gpage.goto(BASE + '/schedule.php', { waitUntil: 'domcontentloaded' });
    const nav = gpage.locator('.site-nav');
    await expect(nav).toBeVisible();
    await expect(nav.locator('.site-nav-link', { hasText: marker })).toHaveClass(/active/);
    ...
    if (it) await api({ action: 'delete_menu_item', id: String(it.id) });
  }
});
